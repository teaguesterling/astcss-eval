...
FILTERS ON A NODE (written directly after it, no space)
  #name                   exactly this name:            .fn#main   .call#print   .class#User
  [name^="x"]             name starts with x            .fn[name^="test_"]
  [name$="x"]             name ends with x
  [name*="x"]             name contains x
  [params=N]              function with exactly N parameters   .fn[params=2]
  :has(S)                 contains a descendant matching S     .fn:has(.call#open)
